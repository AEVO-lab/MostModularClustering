...
# Change log:
# - get_colorful_subsets updated to ML's version
# - get_Yset considers only vertices that have at least half
#   their neighbors in setx
# - fixed bugs for best cluster for v
#.........................................................
...
def cost_f(graph,setx,cost_type='default'):
    ...
    if(cost_type == 'norm'):
        cost_result = cost_result / (10*len(setx)) if len(setx) > 0 else 0 
...
def get_colorful_subsets(graph,vertex):
    ...
    for color in n_colors:
        nbrs = list(graph.nodes[vertex][color])
        nbrs.append(-1)
        choices_per_color.append(nbrs)
    
    choices = product(*choices_per_color)
    retval = []
    for c in choices:
        fc = [x for x in c if x != -1]
        retval.append( fc )
            ...
            for v in graph.neighbors(n):
                if v not in closed_n:
                    if graph.nodes[v]['color'] in c_complement:
                        if v not in some_y:
                            #  don't consider v if it doesn't have at least 1/2 neighbors in Xv
                            cntXv = 0
...
def get_colorful_Yv(graph,yset,c_complement):
    if (len(c_complement) == 0):
        ...
        vertices_per_color = dict()
        for y in yset:
            ycolor = graph.nodes[y]['color']
            if ycolor in c_complement:
                if not ycolor in vertices_per_color:
                    vertices_per_color[ycolor] = [-1, y]
                else:
                    vertices_per_color[ycolor].append(y)
        choices_per_color = []
        for color in vertices_per_color:
            choices_per_color.append(vertices_per_color[color])

        choices = product(*choices_per_color)
        retval = []
        for c in choices:
            retval.append( [x for x in c if x != -1] )
...
#.................................................................           
def most_modular_cluster(graph,cost_t, single_cluster = False, remove_common_neighbors = False):
    ...
    while(flag == True):
        ...
        for v in graph_copy.nodes():
            if graph_copy.nodes[v]['color'] in handledCols:
                continue
            handledCols.add(graph_copy.nodes[v]['color'])
        ...
        #common neighbor cleanup
        if remove_common_neighbors:
            for v in best_c[1]:
                for w in graph_copy.nodes():
                    if v != w and graph_copy.nodes[v]['color'] == graph_copy.nodes[w]['color']:
                        for z in graph_copy.neighbors(v):
                            if z in graph_copy.neighbors(w):
                                graph_copy.nodes[w][graph_copy.nodes[z]['color']].remove(z)
                                graph_copy.nodes[z][graph_copy.nodes[w]['color']].remove(w)
                                graph_copy.remove_edge(w, z)
                                
        remove_cluster(graph_copy,best_c[1])

        singletons_tmp = list(nx.isolates(graph_copy))
        graph_copy.remove_nodes_from(singletons_tmp)
        singletons = singletons + singletons_tmp
        
        if (graph_copy.order() == 0 or graph_copy.number_of_edges() == 0):  #TODO ML
            flag = False
        if single_cluster:
            flag = False
